...
class LongMemEvalAdapter:
    ...
    def conversation_to_memory_events(
        self,
        ...
        answer_session_id_set = set(answer_session_ids or [])
        ...
        for session_idx, session in enumerate(sessions):
            ...
            for msg_idx, message in enumerate(session):
                ...
                event = Event(
                    ...
                    timestamp=timestamp,
                    proposal={
                        'subject': f"{session_id}_msg_{msg_idx}",
                        'predicate': 'utterance',
                        ...
                        'turn_index': msg_idx,
                        'supports_answer': bool(message.get('has_answer')) or session_id in answer_session_id_set,
                    },
                    read_snapshot_time=timestamp,
                )
                events.append(event)
                step += 1
    ...
    def convert_entry_to_scenario(self, entry: Dict[str, Any], num_agents: int = 2) -> Scenario:
        ...
        ordered_events = self.conversation_to_memory_events(
            sessions,
            session_ids,
            answer_session_ids,
            base_timestamp,
        ...
        scenario = Scenario(
            ...
            description=f"{entry['question']} | answer_sessions={','.join(answer_session_ids) if answer_session_ids else 'unknown'}",
            queries=[query],
